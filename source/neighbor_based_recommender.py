...
from .utils import top_k_precision_distribution
...
class neighbor_based_recommender():
    ...
    def predict_ratings(self, neighbor_size):
        ...
        if len(distinct_user) != max_user:
            for seq in range(max_user):
                if seq+1 not in distinct_user:
                    train_data = train_data.append({'userId': seq+1, 'movieId': 1, 'rating': 0.5, 'timestamp': 0},
                                                   ignore_index=True)
        ...
        self.result = activation_matrix.dot(train_data_pivot_user.values) \
    ...
    def RMSE(self):
        ...
        test_data['predictedRating'] = test_data.apply(lambda x: self.rating_prediction(x['userId'], x['movieId']),
                                                       axis=1)
        test_data['squared_error'] = test_data.apply(lambda x: (x['rating'] - x['predictedRating'])**2
